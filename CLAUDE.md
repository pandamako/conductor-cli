...
- **`internal/config/`** — Global config (`~/.conductor-cli/config.json`) load/save, branch name sanitization (URL-encoding: `/` → `%2F`), path helpers. Stateless functions, no global state.
